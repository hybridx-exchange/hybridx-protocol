...
library OrderBookLibrary {
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'OBL: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'OBL: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'OBL: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'OBL: INSUFFICIENT_LIQUIDITY');
    ...
    returns (uint112 reserveA, uint112 reserveB) {
        require(tokenA != tokenB, 'OBL: IDENTICAL_ADDRESSES');
        address token0 = tokenA < tokenB ? tokenA : tokenB;
        require(token0 != address(0), 'OBL: ZERO_ADDRESS');
    ...
    returns (uint amountLeft, uint amountAmmQuote, uint amountQuoteFix) {
        ...
        if (curPrice < targetPrice) {
            ...
            require(_amountLeft >= amountQuoteFix, "OBL: Not Enough Output Amount");
    ...
    returns (uint amountLeft, uint amountAmmBase, uint amountBaseFix) {
        ...
        if (curPrice > targetPrice) {
            ...
            require(_amountLeft >= amountBaseFix, "OBL: Not Enough Input Amount");
